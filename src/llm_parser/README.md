...
```bash
# Parse a single file
python -m llm_parser.cli parse --file data/resume.pdf

# Parse a folder
python -m llm_parser.cli parse --folder data/

# With custom options
python -m llm_parser.cli parse --file resume.pdf --model google/flan-t5-large --no-cache --verbose
```
...
# Run with coverage
pytest --cov=llm_parser --cov-report=html
...
```json
{
  ...
  "level": "INFO",
  "logger": "llm_parser.services.parser_service",
...
1. Review the new architecture in `llm_parser/`
